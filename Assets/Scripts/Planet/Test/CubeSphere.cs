...
public class CubeSphere : MonoBehaviour {
	...
	private void CreateVertices () {
		...
		for (int y = 0; y <= gridSize; y++) {
			for (int x = 0; x <= gridSize; x++) { // Z
                cubeUV[v] = new Vector2(
                    (float)x / (float)gridSize,
                    (float)y / (float)gridSize);
                SetVertex(v++, x, y, 0);
			}
			for (int z = 1; z <= gridSize; z++) { // X
                cubeUV[v] = new Vector2(
                    (float)(z - 1) / ((float)gridSize - 1),
                    (float)y / (float)gridSize);
                SetVertex(v++, gridSize, y, z);
            }
			for (int x = gridSize - 1; x >= 0; x--) { // Y ?
                cubeUV[v] = Vector2.zero;
                SetVertex(v++, x, y, gridSize);
            }
			for (int z = gridSize - 1; z > 0; z--) { // MY ?
                cubeUV[v] = Vector2.zero;
                SetVertex(v++, 0, y, z);
            }
        }
		for (int z = 1; z < gridSize; z++) {
			for (int x = 1; x < gridSize; x++) { // Z ?
                cubeUV[v] = Vector2.zero;
                SetVertex(v++, x, gridSize, z);
            }
        }
		for (int z = 1; z < gridSize; z++) {
			for (int x = 1; x < gridSize; x++) { // MZ ?
                cubeUV[v] = Vector2.zero;
                SetVertex(v++, x, 0, z);
            }
        }
	...
	private void CreateTriangles () {
        ...
        int[] trianglesMZ = new int[(gridSize * gridSize) * 12];
        int[] trianglesMX = new int[(gridSize * gridSize) * 12];
        int[] trianglesMY = new int[(gridSize * gridSize) * 12];
		...
		for (int y = 0; y < gridSize; y++, v++) {
			...
			for (int q = 0; q < gridSize; q++, v++) {
				tZ = SetQuad(trianglesMZ, tZ, v, v + 1, v + ring, v + ring + 1);
			}
			for (int q = 0; q < gridSize - 1; q++, v++) {
				tX = SetQuad(trianglesMX, tX, v, v + 1, v + ring, v + ring + 1);
			}
			tX = SetQuad(trianglesMX, tX, v, v - ring + 1, v + ring, v + 1);
		}

		tY = CreateTopFace(trianglesY, tY, ring);
		tY = CreateBottomFace(trianglesMY, tY, ring);

		mesh.subMeshCount = 6;
        mesh.SetTriangles(trianglesZ, 0);
        mesh.SetTriangles(trianglesMZ, 1);
        mesh.SetTriangles(trianglesX, 2);
        mesh.SetTriangles(trianglesMX, 3);
        mesh.SetTriangles(trianglesY, 4);
        mesh.SetTriangles(trianglesMY, 5);
    }
